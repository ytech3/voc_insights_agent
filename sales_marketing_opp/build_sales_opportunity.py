"""Build Sales Opportunity Excel for any VOC homestand date range.

Usage:
    python build_sales_opportunity.py --start-date 2026-05-01 --end-date 2026-05-06
    python build_sales_opportunity.py --start-date 2026-05-01 --end-date 2026-05-06 --output my_file.xlsx
...
Cohort filters (fixed):
  ...
  - PREVIOUS_PURCHASE_DESC != "Yes" (no prior ticket purchasers)
"""
...
def fetch_data(start_date: str, end_date: str) -> pd.DataFrame:
    """Pull all rows from the view for the given date range."""
    query = f"""
    SELECT *
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build VOC Sales Opportunity Excel for a given date range."
    )
    ...
    parser.add_argument("--output", default=None,
                        help="Output filename (default: VOC_{start}_{end}.xlsx)")
    ...
    if args.output:
        ...
        out_path = SCRIPT_DIR / f"VOC_{start_fmt}_{end_fmt}.xlsx"

    # Step 1: Fetch all data from Snowflake
    df_all = fetch_data(start_date, end_date)

    if len(df_all) == 0:
        print(f"No data found for date range {start_date} to {end_date}. Exiting.")
        return
    ...
    if len(cohort) == 0:
        print("No rows passed cohort filters. Writing Sheet1 only (all respondents).")
        # Still write Sheet1 with all data
        ...
        print(f"Wrote {out_path} (Sheet1 only, {len(df_all):,} rows)")
        return
    ...
    # Step 5: Strip timezone info from datetime columns (Excel doesn't support tz-aware)
    TZ_COLUMNS = ["_FIVETRAN_SYNCED", "SYSTEM_START_DATE", "SYSTEM_END_DATE",
                  "SYSTEM_CREATE_DATE", "SYSTEM_UPDATE_DATE"]
    for col in TZ_COLUMNS:
        if col in df_all.columns:
