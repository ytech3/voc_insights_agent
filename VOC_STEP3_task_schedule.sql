...
BEGIN
    ...
    WHILE (v_remaining > 0) DO
        ...
        -- Insert placeholder for IDs that produced no valid sentences (prevents infinite loop)
        INSERT INTO TBRDP_DW_DEV.IM_RPT.T_OVERALL_FEEDBACK_SENTENCE_LEVEL
            (QUALTRICS_ID, SEASON, GAME_DATE, BUYER_TYPE, SECTION_CODE, ORIGINAL_FEEDBACK, SATISFACTION_RATING,
             SENTENCE_NUMBER, SENTENCE_TEXT, SENTIMENT_CATEGORY, AI_CATEGORY, PARENT_CATEGORY,
             DETAILED_CATEGORY, SENTENCE_LENGTH, NPS_SEGMENT)
        SELECT q.QUALTRICS_ID, q.SEASON, q.GAME_DATE, q.BUYER_TYPE, q.SECTION_CODE,
               q.OVERALL_NUMRAT_OT, q.OVERALL_NUMRAT,
               0, '[NO_VALID_SENTENCES]', 'Unclassifiable', 'Unclassifiable', 'Unclassifiable',
               'Unclassifiable - Unclassifiable', 0,
               CASE WHEN q.OVERALL_NUMRAT >= 9 THEN 'Promoter' WHEN q.OVERALL_NUMRAT >= 7 THEN 'Passive' ELSE 'Detractor' END
        FROM TBRDP_DW_DEV.IM_RPT.V_SBL_QUALTRICS_VOC_POST_ATTENDANCE_FULL_CORTEX_AI q
        INNER JOIN TBRDP_DW_DEV.IM_RPT.TMP_SENTENCE_BATCH b ON q.QUALTRICS_ID = b.QUALTRICS_ID
        WHERE b.QUALTRICS_ID NOT IN (
            SELECT DISTINCT QUALTRICS_ID
            FROM TBRDP_DW_DEV.IM_RPT.T_OVERALL_FEEDBACK_SENTENCE_LEVEL
        );
